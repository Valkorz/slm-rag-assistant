...
class Instructions:
    ...
    def _instPrompt_EN(self, context: str, user_question: str) -> str:
        return (
            ...
            f"- If any source is relevant to the question, you MUST answer from it. Do not refuse.\n"
            f"- Set 'answer' to exactly '{self._errSources_EN()}' (and 'sources' to []) ONLY when none of the sources relate to the question.\n"
            f"- Replace the placeholder text (e.g. \"your final answer here\") with the real answer. NEVER output the placeholder text literally.\n"
            ...
            f"- Output the JSON object only. Write nothing after the closing brace.\n\n"
            f"### SOURCES\n"
            f"{context}\n\n"
            f"### QUESTION\n"
            f"{user_question}\n\n"
    ...
    def _instPrompt_Adaptive_PTBR(self, context: str, user_question: str) -> str:
        return (
            ...
            f"- Substitua o texto de exemplo (ex: \"sua resposta final aqui\") pela resposta real. NUNCA escreva o texto de exemplo literalmente.\n"
            ...
            f"- Produza apenas o objeto JSON. Não escreva nada após a chave de fechamento.\n\n"
            f"### EXEMPLO\n"
    ...
    def _instPrompt_PTBR(self, context: str, user_question: str) -> str:
        return (
            ...
            f"- Se qualquer fonte for relevante para a pergunta, você DEVE responder com base nela. Não se recuse.\n"
            f"- Defina 'answer' como exatamente '{self._errSources_PTBR()}' (e 'sources' como []) SOMENTE quando nenhuma fonte tiver relação com a pergunta.\n"
            f"- Substitua o texto de exemplo (ex: \"sua resposta final aqui\") pela resposta real. NUNCA escreva o texto de exemplo literalmente.\n"
